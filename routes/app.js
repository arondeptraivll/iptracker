...
const { Link, Visit, Key, Credential, sequelize } = require('../models');
...
router.post('/log', async (req, res) => {
    ...
    if (!shortId || !fingerprint) return res.status(400).json({ status: 'error', message: 'Thiếu dữ liệu' });
    ...
    try {
        ...
        const finalFingerprintId = (await Visit.findOne({ where: { fingerprint }, include: [{ model: Link, where: { userDiscordId: link.userDiscordId } }] }))?.fingerprintId || nanoid(10);
        ...
        res.json({ status: 'success', visitId: newVisit.id });
    } catch (error) {
